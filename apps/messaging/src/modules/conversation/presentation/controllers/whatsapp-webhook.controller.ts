...
import { FindWhatsappChannelByPhoneNumberIdQuery } from '@modules/channel-config/application/queries/find-whatsapp-channel-by-phone-number-id/find-whatsapp-channel-by-phone-number-id.query';
...
import { MarkMessageStatusCommand } from '@modules/conversation/application/commands/mark-message-status/mark-message-status.command';
import { RecordWhatsappQualityCommand } from '@modules/channel-config/application/commands/record-whatsapp-quality/record-whatsapp-quality.command';
import { toWhatsappMediaRef } from '@modules/conversation/domain/media-reference';
import { resolveWhatsappError } from '@modules/conversation/domain/whatsapp-error.map';
import {
  ...
  WhatsappValue,
  WhatsappWebhookBody,
} from '../../domain/contracts/whatsapp-webhook.contracts';
import { CryptoManager } from '@src/infrastructure/security/crypto/crypto.manager';
import { InboundMessagePayload } from '@modules/conversation/domain/contracts/message.contracts';
  ...
  private async dispatchStatus(status: WhatsappStatus): Promise<void> {
    ...
    await this.commandBus.execute(
      new MarkMessageStatusCommand({
        ...
        status: mapped,
        errorReason: resolved?.reason ?? null,
