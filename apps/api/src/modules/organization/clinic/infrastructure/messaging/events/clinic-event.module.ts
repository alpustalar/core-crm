import { Module } from '@nestjs/common';
import { ClinicDeletedListener } from '@modules/organization/clinic/infrastructure/messaging/events/listeners';
import { CLINIC_EVENT_PUBLISHER } from '@modules/organization/clinic/domain/interfaces/clinic.event-publisher.interface';
import { ClinicEventPublisher } from '@modules/organization/clinic/infrastructure/messaging/events/clinic-event-publisher.service';
import { MAIL_SERVICE } from '@src/infrastructure/mail/interfaces/mail.service.interface';
import { MailService } from '@src/infrastructure/mail/mail.service';

@Module({
  providers: [
    ClinicDeletedListener,
    {
      provide: CLINIC_EVENT_PUBLISHER,
      useClass: ClinicEventPublisher,
    },
    {
      provide: MAIL_SERVICE,
      useClass: MailService,
    },
  ],
  exports: [CLINIC_EVENT_PUBLISHER],
})
export class ClinicEventModule {}
