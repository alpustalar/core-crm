...
export class PaxRefundHandler implements ICommandHandler<PaxRefundCommand, PaxRefundResponse> {
  ...
  async execute(command: PaxRefundCommand): Promise<PaxRefundResponse> {
    ...
    if (originalTx.status !== PosTransactionStatus.SUCCESS) {
      throw new BadRequestException('Yalnızca başarılı işlemler iade edilebilir.');
    }
    ...
    if (refundAmount > Number(originalTx.amount)) {
      throw new BadRequestException('İade tutarı orijinal işlem tutarını aşamaz.');
    }
    ...
    try {
      ...
      this.logger.log(`PAX iade tamamlandı: id=${refundTransactionId} status=${status}`);
      ...
      if (err instanceof PaxTimeoutError) {
        this.logger.warn(`PAX iade timeout: id=${refundTransactionId} — PENDING kalıyor`);
      ...
      if (err instanceof PaxConnectionError) {
        ...
        this.logger.error(`PAX iade bağlantı hatası: id=${refundTransactionId}`);
