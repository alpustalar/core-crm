...
export class RegisterPosDeviceHandler
  implements ICommandHandler<RegisterPosDeviceCommand, RegisterPosDeviceResponse>
{
  constructor(
  ...
  async execute(command: RegisterPosDeviceCommand): Promise<RegisterPosDeviceResponse> {
    const { input } = command;
    ...
    return { id: device.id, clinicId: device.clinicId, label: device.label, terminalId: device.terminalId };
  }
}
