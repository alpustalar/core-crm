...
import { IPosDeviceCommandRepository } from '@modules/pos/domain/repositories/pos-device.repository';
  ...
  implements IPosDeviceCommandRepository
{
  ...
  deactivate(id: string): Promise<PosDevice> {
    return this.db.posDevice.update({ where: { id }, data: { isActive: false } });
  }

  async softDelete(id: string): Promise<void> {
    await this.db.posDevice.update({ where: { id }, data: { isDeleted: true, isActive: false } });
  }
}
