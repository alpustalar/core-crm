...
export class ReceiveStockHandler implements ICommandHandler<ReceiveStockCommand, string> {
  constructor(
  ...
  async execute(command: ReceiveStockCommand): Promise<string> {
    ...
    if (!policy.isSystemAdmin() && !policy.actorCanManageTargetClinic(clinicId)) {
      throw new ForbiddenException('Bu kliniğe stok girişi yapma yetkiniz yok.');
    }
    ...
    const vatRate = dto.vatRate != null ? new Prisma.Decimal(dto.vatRate) : null;
    const totalAmount = purchasePrice.times(quantity);
    const vatAmount = vatRate ? totalAmount.times(vatRate).dividedBy(100) : null;
