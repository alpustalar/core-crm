...
export class RecordProductUsageHandler implements ICommandHandler<RecordProductUsageCommand, void> {
  constructor(
  ...
  async execute(command: RecordProductUsageCommand): Promise<void> {
    ...
    if (!policy.isSystemAdmin() && !policy.actorCanManageTargetClinic(clinicId)) {
      throw new ForbiddenException('Bu klinikte ürün kullanımı kaydedemezsiniz.');
    }
