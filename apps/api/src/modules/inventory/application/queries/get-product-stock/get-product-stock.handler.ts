...
export class GetProductStockHandler implements IQueryHandler<GetProductStockQuery, GetProductStockResponse> {
  constructor(
  ...
  async execute(query: GetProductStockQuery): Promise<GetProductStockResponse> {
    ...
    if (!policy.isSystemAdmin() && !policy.actorCanManageTargetClinic(clinicId)) {
      throw new ForbiddenException('Bu kliniğin stok bilgisine erişim yetkiniz yok.');
    }
