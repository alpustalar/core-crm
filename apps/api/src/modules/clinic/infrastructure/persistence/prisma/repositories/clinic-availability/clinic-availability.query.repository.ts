  ...
  implements IClinicAvailabilityQueryRepository
{
  ...
  findByClinicAndDay(clinicId: string, dayOfWeek: number): Promise<IClinicAvailability | null> {
    return this.db.clinicAvailability.findUnique({
      where: { clinicId_dayOfWeek: { clinicId, dayOfWeek } },
    });
  }

  findExceptionByClinicAndDate(clinicId: string, date: Date): Promise<IClinicException | null> {
