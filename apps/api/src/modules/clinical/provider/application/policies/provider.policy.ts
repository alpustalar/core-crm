...
const { ADMIN, INTERNAL, DATA_OWNER, MANAGEMENT } = ProviderResponseGroups;
...
export class ProviderPolicy extends UserPolicy {
  getSerializationOptions(
    ...
    if (isManager) groups.push(MANAGEMENT, INTERNAL);
    if (isSameClinic) groups.push(INTERNAL);
    if (isSystem) groups.push(ADMIN, DATA_OWNER, MANAGEMENT, INTERNAL);

    return {
      isGroupActive: isManager || isSameClinic || isSelf || isSystem,
      groups,
    };
  }
}
