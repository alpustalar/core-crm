...
export class AutoMatchStatementLinesHandler implements ICommandHandler<
...
> {
  ...
  async execute(
    ...
    const statement = await this.statementCommandRepo.findById(bankStatementId);
    if (!statement) {
    ...
    return this.txManager.run(async () => {
      const lines =
        await this.lineCommandRepo.findUnmatchedByStatementId(bankStatementId);
      ...
      await this.lineCommandRepo.updateMany(touched);
