  ...
  implements ICommandHandler<UpdateUserByStaffCommand, string>
{
  ...
  async execute(command: UpdateUserByStaffCommand) {
    ...
    const targetUser = await this.userRepo.findOneWithAnIdOrEmail(targetUserId);
    ...
    this.policyFactory
      ...
      .check(
        (p) => p.actorHasHigherPriorityThanTarget(targetUser),
    ...
    // 4. Persistence
    const data = UserCommandsPrismaMapper.toUpdateUserWithAnIdInput(dto);
