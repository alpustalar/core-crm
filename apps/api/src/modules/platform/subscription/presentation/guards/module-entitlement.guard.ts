...
/** Sistem admin eşiği — bu ve üstü öncelikteki roller tüm modüllere erişir (capability guard ile aynı). */
const SYSTEM_ADMIN_PRIORITY = 100;
...
export class ModuleEntitlementGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if ((actor?.role?.priority ?? 0) >= SYSTEM_ADMIN_PRIORITY) return true;
  ...
  private async resolveEntitlements(
    ...
    const cached = await this.redis.getTenantEntitlements(
      organizationId,
      clinicId
    );
    ...
    await this.redis.setTenantEntitlements(organizationId, clinicId, data);
    return data;
  }
}
